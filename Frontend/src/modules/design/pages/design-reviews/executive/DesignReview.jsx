import { useGetAllDesignReviewListQuery } from "@/api/design/common_workflow/design-review/design-review.api";
...
const DesignReview = () => {
  ...
  const onView = (row) => {
    navigate(`/design/executive/designs/review-details/${row.id}`);
  };
  ...
  const columnArray = [
    ...
    { key: "date", label: "Date" },
    {
      ...
      render: (value) => (
        <span
          className={`px-2 py-1 rounded-sm text-xs font-semibold bg-green-100 text-green-600"}`}
        >
          {value}
        </span>
      ),
