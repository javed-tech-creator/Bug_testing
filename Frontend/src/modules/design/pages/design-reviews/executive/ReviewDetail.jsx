...
import { useAddDesignReviewRatingMutation } from "@/api/design/common_workflow/design-review/design-review.api";
...
const managerChecklistInitial = [
  { key: "environment", label: "Environmental Conditions", checked: true },
  { key: "productReq", label: "Product Requirements", checked: true },
  { key: "images", label: "Uploaded Images", checked: true },
  { key: "videos", label: "Uploaded Videos", checked: true },
  { key: "installation", label: "Installation Details", checked: true },
  { key: "rawRecce", label: "Raw Recce", checked: true },
  { key: "clientData", label: "Data From Client", checked: true },
  { key: "measurement", label: "Accurate Measurements Taken", checked: true },
  {
    key: "productName",
    label: "All Signage Name (Product Name) Confirmed",
    checked: true,
  },
  { key: "instructions", label: "Instructions / Remarks", checked: true },
  { key: "sameDay", label: "Submitted Same Day", checked: true },
];
...
const ReviewDetail = () => {
  ...
  const [selfRatingData, setSelfRatingData] = useState({
    rating: 0,
    remark: "",
  });
  ...
  const managerFeedbackDummy = {
    comment:
      "All measurements verified. Design is accurate and ready for quotation.",
    rating: 4,
    decision: "approve", // approve | flag
    flagType: "",
    flagRemark: "",
  };
  useEffect(() => {
    ...
    setFormDataFeedback({
      comment: managerFeedbackDummy.comment,
      rating: managerFeedbackDummy.rating,
      decision: managerFeedbackDummy.decision,
      flagType: managerFeedbackDummy.flagType,
      flagRemark: managerFeedbackDummy.flagRemark,
    });
  }, []);
  ...
  const handleOpenReview = (designId) => {
  ...
  const [submitReview, { isUpading }] = useAddDesignReviewRatingMutation();
  ...
  const handleReviewSubmitBtn = async (e) => {
    ...
    try {
      const payload = {
        _id: id,
        executive_submission: selfRatingData
      }

      console.log('payload:>', payload)

      const res = await submitReview({data:payload}).unwrap();
      toast.success(res?.data?.message ?? res?.message ?? "Submitted successfully.");

      console.log("Self Rating Submitted:", selfRatingData);
      setIsSelfRatingOpen(false);

    } catch (err) {
      console.log('err:>', err)
      toast.error(err?.error?.message ?? err?.data?.message ?? "Failed to submit!")
    }
            ...
            `}
                >
                  <input
                    ...
                    checked={item.checked}
                    disabled
                    className="h-5 w-5 accent-green-600 cursor-not-allowed"
                  />
  ...
  `}
          ...
          {/* Footer */}
          <div className="px-4 py-3 border-t flex justify-end gap-3 mt-4">
            <button
              onClick={() => navigate(-1)}
            ...
            <button
              onClick={() => setIsSelfRatingOpen(true)}
              className="px-4 py-2 text-sm rounded-sm bg-yellow-500 hover:bg-yellow-600 text-white disabled:opacity-50"
            >
              Give Rating Yourself
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white w-full max-w-md rounded-lg shadow-lg">
            ...
            <div className="px-4 py-3 border-b flex justify-between items-center">
              <h2 className="text-lg font-semibold">Give Your Rating</h2>
              <button
                onClick={() => setIsSelfRatingOpen(false)}
                className="
  ...
  "
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      ...
                      size={22}
                      onClick={() =>
                        setSelfRatingData((prev) => ({
                          ...prev,
                          rating: star,
                        }))
                      }
                      className={`cursor-pointer ${star <= selfRatingData.rating
                        ? "fill-yellow-400 text-yellow-400"
                        : "text-gray-300"
                        }`}
                    />
            ...
            <div className="px-4 py-3 border-t flex justify-end gap-3">
              <button
                onClick={() => setIsSelfRatingOpen(false)}
                className="px-4 py-2 text-sm rounded-sm bg-gray-500 text-white"
              >
                Cancel
              </button>
              ...
              <button
                onClick={(e) => {
                  handleReviewSubmitBtn(e)
                }}
                disabled={!selfRatingData.rating || !selfRatingData.remark}
                className="px-4 py-2 text-sm rounded-sm bg-green-600 text-white disabled:opacity-50"
              >
                Submit
              </button>
            </div>
