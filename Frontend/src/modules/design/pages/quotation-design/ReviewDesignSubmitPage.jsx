...
import { useFinalSubmitMeasurementQuotationMutation } from "@/api/design/common_workflow/view-design-option.api";
...
const ReviewDesignSubmitPage = () => {
  ...
  const [measurementFinalSubmit, { isUpdating }] = useFinalSubmitMeasurementQuotationMutation();


  const handleFinalSubmit = async (e) => {
    console.log('1 - selected checked list:>', selectedChecklist);
    e.preventDefault();
    console.log('selected checked list:>', selectedChecklist);

    try {

      const formattedCheckList = Object.entries(selectedChecklist).map(
        ([label, value]) => ({
          label,
          is_checked: value
        })
      ...
      const res = await measurementFinalSubmit({ data: payload }).unwrap();
      console.log('res:>', res)
      toast.success(res?.data?.message ?? res?.message ?? "Submitted successfully.");

    } catch (err) {
      console.log('err:>', err)
      toast.error(err?.error?.message ?? err?.data?.message ?? err?.message ?? "Failed to save.");
    }
