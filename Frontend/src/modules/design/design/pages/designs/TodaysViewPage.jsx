...
import { useUpdateNextDayPlanningMutation } from "@/api/design/manager/next-day-planning/nextdayplanning.api";

const TodaysViewPage = () => {
  ...
  const [updateNextDayPlanning, { isLoading: isUpdating, error: updateError }] = useUpdateNextDayPlanningMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.status == "declined" && !formData.remark) {
      toast.error("Remark is required when decline")
      return;
    }

    const payLoad = {
      approval_status: formData.status == "accepted" ? "accepted" : "rejected",
      remark: formData.remark
    }

    try {
      const res = await updateNextDayPlanning({
        id,
        data: payLoad
      }).unwrap();
  ...
  return (
    <div className="px-5">
      <DesignsHeader
        ...
        recievingDate="15/07/25 - 10:00AM"
      ...
      <div className="space-y-4">
        ...
        {type !== "next-day-planning" && (
          <div className="bg-white border rounded-sm px-2 py-4 flex flex-wrap gap-2 justify-end items-center">
            {/* Download PDF */}
            <button
              className="cursor-pointer px-2 py-1.5 rounded-sm bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition"
            ...
            >
              Download Images/Video ZIP
            </button>

            {type === "today" && (
              <button
                className="px-2 py-1.5 rounded-sm border border-blue-600 text-blue-600 text-sm font-medium hover:bg-blue-100 transition cursor-pointer"
                onClick={() => console.log("Marked as Started")}
              >
                Mark as Started
              </button>
            )}
        ...
        {type === "next-day-planning" && (
          ...
          >
            <h2 className="text-base font-semibold mb-4">ACCEPT / DECLINE</h2>
            <hr className="mb-5" />
            ...
            <div className="flex justify-end mt-6">
              <button
                type="submit"
                disabled={isUpdating}
                className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2 rounded-md text-sm font-medium"
              >
                {isUpdating ?
                  <>
