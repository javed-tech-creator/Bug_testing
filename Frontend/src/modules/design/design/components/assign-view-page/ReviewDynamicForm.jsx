...
const ReviewDynamicForm = ({ isOpen, onClose, title, remark, onSave, ratingNo=0 }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setRating(ratingNo || 0);
    }
  }, [isOpen, remark, ratingNo]);
    ...
    <div className="fixed inset-0 bg-black/20  flex items-center justify-center p-4 z-50">
      ...
      <div className="w-full max-w-lg bg-white rounded-md shadow-lg overflow-hidden border border-gray-200">
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    ...
                    className={`cursor-pointer transition-colors ${star <= (hoveredStar || rating)
        ...
        <div className="flex justify-end gap-2 px-4 py-3 bg-gray-50 border-t border-gray-100">
          ...
          <button
            onClick={handleSave}
            className="px-4 py-2 text-sm font-medium cursor-pointer text-white bg-blue-600 rounded-md hover:bg-blue-700 transition-colors"
          >
            Save
          </button>
        </div>
...
export default ReviewDynamicForm;
