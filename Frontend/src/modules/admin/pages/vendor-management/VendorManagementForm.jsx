...
} from "@/api/admin/vendor-profile-management/vendor.management.api";
...
const VendorManagementForm = () => {
  ...
  const [fetchVendor, { data: vendorData, isLoading: isFetching }] =
    useLazyGetVendorByIdQuery();
  const [createVendor, { isLoading: isCreating }] = useCreateVendorMutation();
  const [updateVendor, { isLoading: isUpdating }] = useUpdateVendorMutation();

  const existingVendor = vendorData?.data;
  ...
  // Fetch vendor if editing
  useEffect(() => {
    if (id) fetchVendor(id);
  }, [id]);

  // Populate existing vendor data
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      let res;
      if (existingVendor) {
        res = await updateVendor({ id: existingVendor._id, data }).unwrap();
        toast.success(res?.message || "Vendor updated successfully!");
      } else {
        res = await createVendor(data).unwrap();
      ...
      toast.error(error?.data?.message || "Error saving vendor!");
      console.error(error);
    }
  };
          ...
          >
            {isCreating || isUpdating ? (
              <>
                ...
                {existingVendor ? "Updating..." : "Saving..."}
              </>
            ) : existingVendor ? (
              "Update"
            ) : (
