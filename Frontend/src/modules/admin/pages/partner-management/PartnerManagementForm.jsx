...
} from "@/api/admin/partner-profile-management/partner.management.api";
...
const PartnerManagementForm = () => {
  ...
  const [fetchPartner, { data: partnerData, isLoading: isFetching }] =
    useLazyGetPartnerByIdQuery();
  ...
  const existingPartner = partnerData?.data;
  ...
  // Populate existing Partner data
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      let res;
      if (existingPartner) {
        res = await updatePartner({ id: existingPartner._id, data }).unwrap();
        toast.success(res?.message || "Partner updated successfully!");
      } else {
        res = await createPartner(data).unwrap();
      ...
      toast.error(error?.data?.message || "Error saving Partner!");
      console.error(error);
    }
  };
          ...
          >
            {isCreating || isUpdating ? (
              <>
                ...
                {existingPartner ? "Updating..." : "Saving..."}
              </>
            ) : existingPartner ? (
              "Update"
            ) : (
