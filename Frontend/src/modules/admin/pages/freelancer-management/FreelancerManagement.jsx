...
import { useGetAllFreelancersQuery } from "@/api/admin/freelancer-profile-management/freelancer.management.api";
...
const [itemsPerPage, setItemsPerPage] = useState(25);
  ...
  const handleAdd = () => navigate("/admin/freelancer-management/form");

  const { data, isLoading, isFetching, error } = useGetAllFreelancersQuery({
    page: currentPage,
    limit: itemsPerPage,
    isActive,
  });
  ...
  const handleEdit = (row) => {
    navigate(`/admin/freelancer-management/form/${row._id}`);
  };

  const handleView = (row) => {
    console.log("freelancer details", row);

    navigate(`/admin/freelancer-management/profile/${row._id}`);
  };

  const freelancerColumns = [
    { key: "profileId", label: "Freelancer ID" },
    {
      key: "actions",
      label: "Actions",
      render: (_, row) => (
  ...
  return (
    <div className="p-6 max-w-9xl mx-auto space-y-6">
      ...
      <DynamicTableList
        columnArray={freelancerColumns}
        tableData={freelancers}
        total={data?.total}
