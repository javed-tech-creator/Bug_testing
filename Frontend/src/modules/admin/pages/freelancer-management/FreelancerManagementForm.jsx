...
} from "@/api/admin/freelancer-profile-management/freelancer.management.api";
...
const FreelancerManagementForm = () => {
  const navigate = useNavigate();
  const { id } = useParams();

  const [fetchFreelancer, { data: freelancerData, isLoading: isFetching }] =
    useLazyGetFreelancerByIdQuery();
  ...
  const existingFreelancer = freelancerData?.data;
  ...
  // Fetch Freelancer if editing
  useEffect(() => {
    if (id) fetchFreelancer(id);
  }, [id]);

  // Populate existing Freelancer data
  useEffect(() => {
    if (!existingFreelancer) return;
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (existingFreelancer) {
        res = await updateFreelancer({ id: existingFreelancer._id, data }).unwrap();
        toast.success(res?.message || "Freelancer updated successfully!");
      } else {
        res = await createFreelancer(data).unwrap();
        toast.success(res?.message || "Freelancer created successfully!");
      ...
      toast.error(error?.data?.message || "Error saving Freelancer!");
      console.error(error);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Complete Freelancer information form
              </p>
            </div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            {renderInput({
              ...
              name: "email",
              type: "email",
              placeholder: "freelancer@company.com",
              icon: Mail,
            })}
          </div>
          <div className="mt-5">
          ...
          >
            {isCreating || isUpdating ? (
              <>
                ...
                {existingFreelancer ? "Updating..." : "Saving..."}
              </>
            ) : existingFreelancer ? (
              "Update"
            ) : (
...
export default FreelancerManagementForm;