...
} from "@/api/admin/contractor-profile-management/contractor.management.api";
...
const ContractorManagementForm = () => {
  const navigate = useNavigate();
  const { id } = useParams();

  const [fetchContractor, { data: contractorData, isLoading: isFetching }] =
    useLazyGetContractorByIdQuery();
  ...
  const existingContractor = contractorData?.data;
  ...
  // Fetch Contractor if editing
  useEffect(() => {
    if (id) fetchContractor(id);
  }, [id]);

  // Populate existing Contractor data
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (existingContractor) {
        res = await updateContractor({ id: existingContractor._id, data }).unwrap();
        toast.success(res?.message || "Contractor updated successfully!");
      } else {
        res = await createContractor(data).unwrap();
        toast.success(res?.message || "Contractor created successfully!");
      ...
      toast.error(error?.data?.message || "Error saving Contractor!");
      console.error(error);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Complete Contractor information form
              </p>
            </div>
          </div>
          ...
          >
            {isCreating || isUpdating ? (
              <>
                ...
                {existingContractor ? "Updating..." : "Saving..."}
              </>
            ) : existingContractor ? (
              "Update"
            ) : (
...
export default ContractorManagementForm;
