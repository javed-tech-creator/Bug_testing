...
const UserProfile = ({ title, id, fetchById }) => {
  ...
  const apiResponse = data?.data?.user;
  ...
  const sections = [
    {
      ...
      fields: [
        { label: "Contact Person Name", value: apiResponse?.name },
        ...(apiResponse?.type !== "EMPLOYEE"
          ? [
              {
                label: "Business Name",
                value: apiResponse?.profile?.businessName,
              },
            ]
          : []),
        { label: "Email", value: apiResponse?.email },
        { label: "Contact Number", value: apiResponse?.phone },
        { label: "Alternate Contact", value: apiResponse?.altPhone },
      ],
    },
    {
      title: "Address",
      fields: [
        {
          label: "Address",
          value:
            apiResponse?.profile?.address ||
            apiResponse?.profile?.currentAddress,
        },
        { label: "City", value: apiResponse?.profile?.city },
        { label: "State", value: apiResponse?.profile?.state },
        { label: "Pincode", value: apiResponse?.profile?.pincode },
      ],
    },

    {
      title: "Work Location",
      fields: [
        // { label: "designation", value: apiResponse?.designation?.title },
        { label: "department", value: apiResponse?.department?.title },
        { label: "branch", value: apiResponse?.branch?.title },
        { label: "city", value: apiResponse?.city?.title },
        { label: "state", value: apiResponse?.state?.title },
        { label: "zone", value: apiResponse?.zone?.title },
      ],
    },
      ...
      fields: [
        ...(apiResponse?.type !== "EMPLOYEE"
          ? [
              {
                label: "GST Number",
                value: apiResponse?.profile?.gstNumber,
              },
              {
                label: "PAN Number",
                value: apiResponse?.profile?.panNumber,
              },
              {
                label: "Aadhar Number",
                value: apiResponse?.profile?.aadharNumber,
              },
            ]
          : [
              {
                ...
                value:
                  apiResponse?.profile?.bankDetail?.accountHolderName || "",
              },
              {
                label: "Branch Name",
                value: apiResponse?.profile?.bankDetail?.branchName || "",
              },
            ]),
        {
          label: "Bank Name",
          value:
            apiResponse?.profile?.bankName ||
            apiResponse?.profile?.bankDetail?.bankName,
        },
        {
          label: "Account Number",
          value:
            apiResponse?.profile?.accountNumber ||
            apiResponse?.profile?.bankDetail?.accountNumber,
        },
        {
          label: "IFSC Code",
          value:
            apiResponse?.profile?.ifscCode ||
            apiResponse?.profile?.bankDetail?.ifscCode,
        },
      ],
    },

    ...(apiResponse?.manageBy
      ? [
          {
            title: "ManageBY (Employee)",
            fields: [
              { label: "Name", value: apiResponse?.manageBy?.name },
              { label: "Email", value: apiResponse?.manageBy?.email },
              { label: "Phone", value: apiResponse?.manageBy?.phone },
              {
                label: "Designation",
                value: apiResponse?.manageBy?.designation?.title,
              },
              { label: "UserId", value: apiResponse?.manageBy?.userId },
            ],
          },
        ]
      : []),
  ...
  return (
    <div className="min-h-screen bg-white p-4 sm:p-6">
      <div className="max-w-8xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-md shadow-lg border border-gray-200 p-2 sm:p-4">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800">
                  {apiResponse?.type &&
                    apiResponse.type.charAt(0).toUpperCase() +
                      apiResponse.type.slice(1).toLowerCase()}{" "}
                  Profile
                </h2>
            ...
            <button
              onClick={() => navigate(-1)}
              className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-gray-700 to-gray-800 text-white text-sm font-semibold rounded-xl shadow-lg  hover:scale-105 transition-all duration-200"
            >
              <ArrowLeft size={18} />
              Back
            </button>
            ...
            <div className="bg-white h-full rounded-md shadow-lg border border-gray-200 p-6  transition-all duration-300">
              <div className="flex flex-col items-center gap-4">
                <div className="relative group">
                  <div className="absolute inset-0 bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 rounded-full blur-xl opacity-40 group-hover:opacity-60 transition-opacity animate-pulse"></div>
                  <img
                    src={
                      apiResponse.profile?.profileImage?.public_url ||
                      apiResponse.profile?.profileImage?.url ||
                      apiResponse.profile?.photo?.public_url ||
                      apiResponse.profile?.photo?.url ||
                      "https://i.pravatar.cc/100?img=13"
                    }
                    alt="Profile"
                    className="relative w-32 h-32 rounded-full border-4 border-white shadow-2xl object-cover ring-4 ring-indigo-100"
                  />
                </div>
                  ...
                  {apiResponse?.type !== "EMPLOYEE" ? (
                    <div className="pt-3 border-t border-gray-200">
                      ...
                      <p className="text-sm font-semibold text-gray-700">
                        {apiResponse?.profile?.businessName || "—"}
                      </p>
                    </div>
                  ) : (
                    <div className="pt-3 border-t border-gray-200">
        ...
        <div className="bg-white rounded-md shadow-lg border border-gray-200 p-6  transition-all duration-300">
          <div className="flex items-center gap-3 border-b border-gray-200 pb-4 mb-6">
            ...
            <h3 className="text-xl font-bold text-gray-800">Work Location</h3>
          </div>
        ...
        {/* Contract Form */}
        {apiResponse?.profile?.contractForm && (
          <div className="bg-white rounded-md shadow-lg border border-gray-200 p-6  transition-all duration-300">
            <div className="flex items-center gap-3 border-b border-gray-200 pb-4 mb-6">
              <div className="w-11 h-11 bg-gradient-to-br from-green-500 to-blue-600  rounded-xl flex items-center justify-center shadow-md">
            ...
            <div className="flex items-center justify-between bg-gray-50 rounded-md p-4 border border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <p className="text-gray-800 font-medium">
                  {apiResponse.profile?.contractForm?.fileName || "No file uploaded"}
                </p>
              </div>

              {apiResponse.profile?.contractForm?.public_url ||
              apiResponse.profile?.contractForm?.url ? (
                  ...
                  <button
                    onClick={() =>
                      handleDownload(
                        apiResponse.profile?.contractForm?.public_url ||
                          apiResponse.profile?.contractForm?.url,
                        apiResponse.profile?.contractForm?.fileName
                      )
                    }
      ...
      {Array.isArray(apiResponse.profile?.additionalDocs) &&
  apiResponse.profile?.additionalDocs.length > 0 && (
    <div className="bg-white rounded-md shadow-lg border border-gray-200 p-6 transition-all duration-300">

      <div className="flex items-center gap-3 border-b border-gray-200 pb-4 mb-6">
        <div className="w-11 h-11 bg-gradient-to-br from-teal-500 to-cyan-600 rounded-xl flex items-center justify-center shadow-md">
          <FileText className="w-6 h-6 text-white" />
        </div>
        <h3 className="text-xl font-bold text-gray-800">
          Additional Documents
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {apiResponse.profile?.additionalDocs?.map((doc, index) => (
          <div
            key={doc._id || index}
            className="flex items-center justify-between bg-gray-50 rounded-md p-4 border border-gray-200 hover:bg-gray-100 transition-colors"
          >
        ...
        {Array.isArray(apiResponse.profile?.documents) &&
          apiResponse.profile?.documents.length > 0 && (
            <div className="bg-white rounded-md shadow-lg border border-gray-200 p-6 transition-all duration-300">
              <div className="flex items-center gap-3 border-b border-gray-200 pb-4 mb-6">
                <div className="w-11 h-11 bg-gradient-to-br from-teal-500 to-cyan-600 rounded-xl flex items-center justify-center shadow-md">
                  <FileText className="w-6 h-6 text-white" />
              ...
              {/* 2 COLUMN GRID + NUMBERING */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {apiResponse.profile?.documents?.map((doc, index) => (
                  <div
                    key={doc._id || index}
                    ...
                    {(doc.public_url || doc.url) && (
                      <div className="flex gap-3">
                        ...
                        <button
                          ...
                          title="Download Document"
                          disabled={downloadingFile === doc.type}
                          className="flex items-center justify-center w-8 h-8  bg-green-100 text-green-600 rounded-sm hover:bg-green-200 transition-all disabled:opacity-50 cursor-pointer"
                        >
                          {downloadingFile === doc.type ? (
                            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                          ) : (
                            <Download size={16} />
                          )}
