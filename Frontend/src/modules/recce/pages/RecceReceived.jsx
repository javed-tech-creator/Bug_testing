...
const RecceReceived = () => {
  ...
  const handleViewDetails = (item) => {
    navigate(`/recce/received-recce-details/${item.id}`, {
      state: {
        recce: item,
        from: "received",
      },
  ...
  const {
    ...
    { skip: isManager } //  skip if manager
  );

  // Common API response
  const apiData = isManager ? managerData : executiveData;
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mb-6">
        ...
        <DesignSimpleHeader title="Recce Received" />
      </div>
