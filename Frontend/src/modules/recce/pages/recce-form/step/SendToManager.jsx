...
const SendToManager = () => {
  ...
  const currentDate = new Date().toLocaleString('en-IN', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
...
const isRecceReport = fromSource === "recce-review-manager";
  ...
  const handleSubmit = () => {
    ...
    if (data && data._id) {
      navigate(`/recce/recce-review-manager/${data._id}`);
    } else {
      navigate(`/recce/recce-review-manager`);
    }
  };
    ...
    <div className="">
      ...
      <div className="rounded-sm shadow-sm border bg-white">
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
          {[
            { key: "toolsReady", label: "Tools Ready" },
            { key: "formFilled", label: "Form Completely Filled" },
            { key: "clientInteraction", label: "Client Interaction Done" },
            { key: "gpsCamera", label: "GPS Camera ON" },
            { key: "clearMarking", label: "Clear Marking Photo Taken" },
            { key: "minPhotos", label: "Minimum 6 Photos Uploaded" },
            { key: "video360", label: "360° Video Uploaded" },
            { key: "accurateMeasurements", label: "Accurate Measurements Taken" },
            { key: "dimensionsChecked", label: "Dimensions Double-Checked" },
            { key: "recceInOut", label: "Recce In & Out Photo Uploaded" },
            { key: "submittedSameDay", label: "Submitted Same Day" },
          ].map(({ key, label }) => (
            <label
              key={key}
  ...
  <div className="bg-white p-5 rounded-sm shadow-sm border border-gray-200">
    ...
    <label className="flex items-start gap-3 p-3 bg-gray-50 rounded-md border border-gray-200 mb-3 cursor-pointer">
      <input
        type="checkbox"
        checked={isConfirmed}
    ...
    <div className="mb-6 flex items-center gap-2 text-xs text-gray-700">
