...
import { ArrowLeft, MapPin, Upload, X, Plus, Check, Eye } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useParams } from "react-router-dom";
import { Dialog } from "@headlessui/react";
// import { useGetAssignedRecceByIdQuery } from "@/api/admin/assignedRecce.api"; // REMOVED API
// import PageHeader from "@/components/PageHeader"; // Assuming this might be used elsewhere, keeping import pattern
...
const PlanningRecceDetails = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  
  // --- API DATA REMOVED AND REPLACED WITH STATIC LOGIC ---
  // const { data: recceDetails, isLoading } = useGetAssignedRecceByIdQuery(id);

  // useEffect(() => {
  //   if (recceDetails?.data) {
  //      // (Previous API mapping logic commented out)
  //   }
  // }, [recceDetails]);
  ...
  const discussionLogs = [
    ...
    },
    {
      ...
      dept: "Recce Dept.",
      repName: "Aman",
      designation: "Recce Executive",
      clientRepName: "Satyam",
  ...
  // Modal States
  const [isPlanningLogModalOpen, setIsPlanningLogModalOpen] = useState(false);
  const [isClientDiscussionModalOpen, setIsClientDiscussionModalOpen] = useState(false);
  const [selectedPlanningLog, setSelectedPlanningLog] = useState(null);

  const handleViewPlanningLog = (log) => {
    setSelectedPlanningLog(log);
    setIsPlanningLogModalOpen(true);
  };

  const closePlanningLogModal = () => {
    setIsPlanningLogModalOpen(false);
    setSelectedPlanningLog(null);
  };

  const openClientDiscussionModal = () => {
    setIsClientDiscussionModalOpen(true);
  };

  const closeClientDiscussionModal = () => {
    setIsClientDiscussionModalOpen(false);
  };


  // if (isLoading) return <div>Loading Recce Details...</div>; // COMMENTED OUT LOADING STATE
    ...
    <div className="min-h-screen">
      ...
      <div className="bg-white p-3 shadow-sm flex justify-between items-center mb-4 rounded-t-lg">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-800">Planing Recce Details</h1>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-4 rounded shadow-sm relative">
            ...
            <div className="w-full h-32 bg-blue-50 border border-blue-100 rounded flex items-center justify-center overflow-hidden relative">
              ...
              {/* Changed z-10 to relative to prevent overlap with header on scroll */}
      ...
      {/* --- NEW SECTIONS AT THE BOTTOM (Full Width) --- */}
      <div className="space-y-4 mt-4">
        ...
        <div className="bg-white p-4 rounded shadow-sm">
          <div className="flex justify-between items-center mb-4">
            ...
            <button 
              onClick={openClientDiscussionModal}
            ...
            <table className="min-w-full text-sm text-left">
              <thead className="bg-gray-800 text-white">
                <tr>
                  ...
                  <th className="px-4 py-2 font-medium">Planner Name</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {planningLogs.map((log) => (
                  <tr key={log.id} className="bg-white hover:bg-gray-50">
                    <td className="px-4 py-3 font-medium text-gray-900">{log.id}</td>
                    <td className="px-4 py-3 text-gray-700">{log.dateTime}</td>
                    <td className="px-4 py-3 text-gray-700">{log.planner}</td>
                    <td className="px-4 py-3 text-gray-700">{log.designation}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex px-2 py-1 rounded text-xs font-medium bg-red-100 text-red-600">
                        {log.leadType}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <button 
                        onClick={() => handleViewPlanningLog(log)}
      ...
      {/* Planning Log View Modal */}
      <Dialog 
        open={isPlanningLogModalOpen} 
        onClose={closePlanningLogModal} 
          ...
          <Dialog.Panel className="w-full max-w-4xl rounded-xl bg-white p-6 shadow-xl">
            <div className="flex justify-between items-center mb-5">
              <Dialog.Title className="text-xl font-semibold text-gray-800">
                Recce Planning Log Details
              </Dialog.Title>
              <button
                onClick={closePlanningLogModal}
            ...
            {selectedPlanningLog && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label>Discussion Date & Time</Label>
                    <Input value="11/12/25 10:00 AM" readOnly={true} className="bg-gray-50" />
                  </div>
                  <div>
                    <Label>Discussion with Client</Label>
                    <Input 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label>Discussion Date & Time</Label>
                    <Input value="12/12/25 10:00 AM" readOnly={true} className="bg-gray-50" />
                  </div>
                  <div>
                    <Label>Discussion with Client</Label>
                    <Input 
                ...
                <div>
                  <Label>Remark</Label>
                  <div className="w-full text-sm border border-gray-200 rounded px-3 py-2 text-gray-600 bg-gray-50 min-h-[100px]">
            ...
            <div className="mt-6 flex justify-end">
              <button
                onClick={closePlanningLogModal}
                className="bg-gray-600 text-white px-5 py-2 rounded-md text-sm font-medium hover:bg-gray-700 transition"
              >
                Close
              </button>
            </div>
      ...
      <Dialog 
        open={isClientDiscussionModalOpen} 
        onClose={closeClientDiscussionModal} 
        className="relative z-50"
      >
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm" aria-hidden="true" />
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="w-full max-w-7xl rounded-xl bg-white p-6 shadow-xl max-h-[90vh] overflow-auto">
            <div className="flex justify-between items-center mb-5">
              <Dialog.Title className="text-xl font-semibold text-gray-800">
                Complete Client Discussion Log
              </Dialog.Title>
              <button
                onClick={closeClientDiscussionModal}
                className="cursor-pointer hover:bg-gray-300 rounded-full p-1 text-gray-600 hover:text-red-500"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="overflow-x-auto border border-gray-200 rounded">
              <table className="min-w-full text-xs text-left">
                <thead className="bg-gray-900 text-white">
                  <tr>
                    <th className="px-3 py-3 font-medium border-r border-gray-700">S.NO</th>
                    <th className="px-3 py-3 font-medium border-r border-gray-700">Department</th>
                    <th className="px-3 py-3 font-medium border-r border-gray-700">Representative Name</th>
                    <th className="px-3 py-3 font-medium border-r border-gray-700">Designation</th>
                    <th className="px-3 py-3 font-medium border-r border-gray-700">Client / Representative Name</th>
                    <th className="px-3 py-3 font-medium w-1/5">Client Designation</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {discussionLogs.map((log) => (
                    <tr key={log.id} className="bg-white hover:bg-gray-50 align-top">
                      <td className="px-3 py-3 font-medium text-gray-900 border-r">{log.id}</td>
                      <td className="px-3 py-3 text-gray-700 border-r">{log.dept}</td>
                      <td className="px-3 py-3 text-gray-700 border-r">{log.repName}</td>
                      <td className="px-3 py-3 text-gray-700 border-r">{log.designation}</td>
                      <td className="px-3 py-3 text-gray-700 border-r">{log.clientRepName}</td>
                      <td className="px-3 py-3 text-gray-600 leading-relaxed">{log.clientDesignation}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export default PlanningRecceDetails;