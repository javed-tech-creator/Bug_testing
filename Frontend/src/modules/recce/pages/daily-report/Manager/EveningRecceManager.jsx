...
/* ================= SAME OPTIONS AS EVENING REPORT ================= */
const durationOptions = Array.from({ length: 16 }, (_, i) => (i + 1) * 0.5);

const dummyEveningRecceData = [
  {
    id: 1,
    ...
    remark: "Morning planned inspection",

    photosTaken: 50,
    measurementDone: 50,
    documentsCollected: 50,
...
/* ================= COMPONENT ================= */
const EveningRecceManager = () => {
  const [now, setNow] = useState(new Date());
  const [reportingMode, setReportingMode] = useState("self");
  const [recceReport, setRecceReport] = useState(dummyEveningRecceData);
  ...
  const handleSubmit = async () => {
    const submitPayload = {
      reportType: "evening",
      selfReport: recceReport,
      teamReport: teamReportData,
      submittedAt: new Date().toISOString(),
    };

    console.log("Submitting Evening Recce Report:", submitPayload);

    try {
      // TODO: Replace with actual API endpoint
      // const response = await fetch('/api/recce/evening-report', {
      //   method: 'POST',
      ...
      alert("Evening recce report submitted successfully!");
    } catch (error) {
  ...
  const filteredReports = recceReport.filter(
    (r) =>
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <PageHeader title="Recce Manager - Evening Report" />
      ...
      {reportingMode === "self" ? (
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-5 bg-white border rounded-md p-4">
            <div className="flex justify-between mb-3">
              <h3 className="font-medium">Today’s Planned Sites</h3>
          ...
          {/* RIGHT — EXACT SAME AS EveningReport */}
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          step="0.5"
                          onChange={(e) =>
                            updateReport(site.id, "timeSpent", e.target.value)
                          }
                          className="w-full border rounded px-2 py-1"
                        />
                      </div>
        ...
        <EveningTeam onDataChange={setTeamReportData} />
      )}
...
export default EveningRecceManager;
