...
// Bottom Sheet Widget
class AssignLicenseBottomSheet extends StatefulWidget {
  ...
  const AssignLicenseBottomSheet({
    Key? key,
    this.onReassign,
    this.onAssetDeleted,
  ...
  @override
  State<AssignLicenseBottomSheet> createState() => _AssignLicenseBottomSheetState();
}

class _AssignLicenseBottomSheetState extends State<AssignLicenseBottomSheet> {
  ...
  Future<void> _handleAssignAsset() async {
    ...
    try {
      ...
      // Call the API
      await assetAssignProvider.assignTechLicenseSoftware(
          context,
          body,
          widget.assetId
      );
      ...
      if (assetAssignProvider.assignTechLicenseModelResponse?.success == true) {
        ...
        if (contextToUse.mounted) {
          try {
            await assetAssignProvider.getAllTechLicenceSoftwareList(context, 1, 5);
        ...
        print('Assignment failed: ${assetAssignProvider.assignTechLicenseModelResponse?.message}');
      }
    } catch (e) {
