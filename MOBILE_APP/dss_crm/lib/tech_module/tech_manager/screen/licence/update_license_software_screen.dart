...
class UpdateLicenseSoftwareScreen extends StatefulWidget {
  final Data productData; // existing product data

  const UpdateLicenseSoftwareScreen({Key? key, required this.productData})
      : super(key: key);

  @override
  State<UpdateLicenseSoftwareScreen> createState() => _UpdateLicenseSoftwareScreenState();
}

class _UpdateLicenseSoftwareScreenState extends State<UpdateLicenseSoftwareScreen> {
  ...
  void _submitForm(TechManagerApiProvider provider) {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      provider.updateTechLicenseSoftware(context, body,assetId ?? "");
    }
              ...
              children: [
                ...
                Consumer<TechManagerApiProvider>(
                  builder: (context, addAssetProvider, child) {
                    return CustomButton(
                      color: Colors.black,
                      text: addAssetProvider.isLoading ? 'Updating...' : "Update Licence",
