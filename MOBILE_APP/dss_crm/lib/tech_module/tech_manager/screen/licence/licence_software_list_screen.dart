...
import 'package:dss_crm/tech_module/tech_manager/widget/asset_detail_bottom_sheet_widget.dart';
import 'package:dss_crm/tech_module/tech_manager/widget/licence_software_detail_bottom_sheet_widget.dart';
import 'package:dss_crm/tech_module/tech_manager/widget/tech_licence_software__list_card_item_widget.dart';
...
import '../../widget/tech_asset__list_card_item_widget.dart';
import '../../widget/tech_asset_filter_sheet_widget.dart';

class LicenceSoftwareListScreen extends StatefulWidget {
  const LicenceSoftwareListScreen({Key? key}) : super(key: key);

  @override
  State<LicenceSoftwareListScreen> createState() => _LicenceSoftwareListScreenState();
}

class _LicenceSoftwareListScreenState extends State<LicenceSoftwareListScreen> {
  ...
  void initState() {
    super.initState();
    // _loadAssets();
    loadLIcenceSoftware(isInitial: true);
  ...
  Future<void> loadLIcenceSoftware({bool isInitial = false}) async {
    ...
    try {
      await provider.getAllTechLicenceSoftwareList(context, _currentPage, _itemsPerPage);

      final response = provider.getAllTechLicenceSoftwareListModelResponse;
  ...
  Future<void> _loadMoreAssets() async {
    ...
    await loadLIcenceSoftware();
  }

  Future<void> refreshAssets() async {
    await loadLIcenceSoftware(isInitial: true);
  }
    ...
    setState(() {
      ...
      if (_searchQuery.isNotEmpty) {
        filteredList = filteredList.where((asset) {
          ...
          return (asset.softwareName?.toLowerCase().contains(searchLower) ?? false) ||
              (asset.versionType?.toLowerCase().contains(searchLower) ?? false) ||
              (asset.licenseId?.toLowerCase().contains(searchLower) ?? false) ||
              (asset.assignedTo?.toLowerCase().contains(searchLower) ??  false) ||
              (asset.department?.toLowerCase().contains(searchLower) ??  false)  ;
        }).toList();
      }
      ...
      if (_currentFilter != null) {
        ...
        if (_currentFilter!.brand != null) {
          filteredList = filteredList
              .where(
                (asset) =>
                    asset.softwareName?.toLowerCase() ==
        ...
        //       .where(
        //         (asset) =>
        //             (asset.amcContract?.toLowerCase() == 'yes') ==
        ...
        actions: [
          ...
          IconButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => TechMangerAddLicenseSoftwareScreen()),
              ).then((_) {
  ...
  Widget _buildAssetsList() {
    // Check if the list of filtered assets is empty
    if (_filteredAssets.isEmpty) {
      // If the list is empty, show the "No assets found" message.
      // We wrap this content in a SingleChildScrollView to prevent pixel overflow
      // when the keyboard is open and reduces the available screen space.
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return LicenceSoftwareItemCard(asset: asset, onTap: () => _showAssetDetails(asset), onAssignmentCompleted: () {
          // यहाँ refresh method call करें
          refreshAssets();
        },);
      },
    );
  ...
  void _showAssetDetails(Data asset) {
    showModalBottomSheet(
      ...
      builder: (context) => LicenceSoftwareDetailBottomSheet(
        asset: asset,
