...
class UpdateAssetScreen extends StatefulWidget {
  ...
  const UpdateAssetScreen({Key? key, required this.productData})
      : super(key: key);
...
class _UpdateAssetScreenState extends State<UpdateAssetScreen> {
  ...
  void _initializeFormFields() {
    ...
    assetId = asset.sId ?? '';
    assetTagController.text = asset.tag ?? '';
    brandController.text = asset.brand ?? '';
    modelSerialController.text = asset.model ?? '';
    vendorSupplierController.text = asset.vendorName ?? '';
    contractNoController.text = asset.contractNo ?? '';

    // Initialize Dropdowns
    selectedAssetType = asset.type;
    selectedLocation = asset.location;
    selectedStatus = asset.status;
    selectedAMCContract = asset.amcContract;

    // Initialize Date Controllers and DateTime objects
    if (asset.purchaseDate != null) {
      _purchaseDate = DateTime.parse(asset.purchaseDate!);
      purchaseDateController.text = DateFormat('dd-MM-yyyy').format(_purchaseDate!);
    }
    if (asset.warrantyEnd != null) {
      _warrantyEndDate = DateTime.parse(asset.warrantyEnd!);
      warrantyEndDateController.text = DateFormat('dd-MM-yyyy').format(_warrantyEndDate!);
    }
    if (asset.validity != null) {
      _validityDate = DateTime.parse(asset.validity!);
      validityController.text = DateFormat('dd-MM-yyyy').format(_validityDate!);
    }
  }
  ...
  void _submitForm(TechManagerApiProvider provider) {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      provider.updateTechAssets(context, body,assetId ?? "");
    }
              ...
              children: [
                ...
                Consumer<TechManagerApiProvider>(
                  builder: (context, addAssetProvider, child) {
                    return CustomButton(
                      color: Colors.black,
                      text: addAssetProvider.isLoading ? 'Updating...' : "Update Asset",
