...
class _UpdateDeviceScreenState extends State<UpdateDeviceScreen> {
  ...
  void _initializeFormFields() {
    ...
    assetId = asset.sId ?? '';
    deviceIdController.text = asset.deviceId ?? '';
    iPAddressController.text = asset.ipAddress ?? '';
    macAddressController.text = asset.macAddress.toString() ?? '';
    installedLocationController.text = asset.installedLocation ?? '';
    vendorAMCPartnerController.text = asset.vendor ?? '';
    configurationDetailsController.text = asset.configurationDetails ?? '';
    maintenanceHistoryController.text = asset.maintenanceHistory ?? '';
    selectedDeviceType = asset.deviceType;
    if (asset.nextServiceDue != null) {
      _nextServiceDueDateDate = DateTime.parse(asset.nextServiceDue!);
      nextServiceDueDateController.text = DateFormat('dd-MM-yyyy').format(_nextServiceDueDateDate!);
    }
  }
  ...
  void _submitForm(TechManagerApiProvider provider) {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      provider.updateTechDevice(context, body, assetId.toString()).then((_) {
        // Call the callback if provided and update was successful
        if (provider.updateTechDeviceModelResponse?.success == true) {
