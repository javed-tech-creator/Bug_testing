...
class TechEngineerDashboardScreen extends StatefulWidget {
  const TechEngineerDashboardScreen({super.key});

  @override
  State<TechEngineerDashboardScreen> createState() => _TechEngineerDashboardScreenState();
}

class _TechEngineerDashboardScreenState extends State<TechEngineerDashboardScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<TechEngineerApiProvider>(
        context,
        listen: false,
      ).getTechEngineerDashboardTopCardData(context);
      // Provider.of<TechEngineerApiProvider>(
      //   context,
      //   listen: false,
  ...
  Future<void> _fetchTopCardData() async {
    final apiProvider = Provider.of<TechEngineerApiProvider>(
      context,
      listen: false,
    );
    apiProvider.getTechEngineerDashboardTopCardData(context);
  }
  ...
  Future<void> _refreshDashboard() async {
    try {
      ...
      final vendorApiProvider = Provider.of<TechEngineerApiProvider>(
        context,
        listen: false,
      );
...
// Data model for Quick Action
  ...
  Widget _buildStatsSection() {
    ...
    return Consumer<TechEngineerApiProvider>(
      builder: (context, apiProvider, child) {
        // Determine the data to show based on the loading state
        final data =
            apiProvider.getTechEngineerDashboardTopCardDataModelResponse?.data;
        ...
        final inProgressTickets = isLoading
            ? '0'
            : '${data?.data?.progressTickets?.toString() ?? '0'}';
        final onHoldTickets = isLoading
            ? '0'
            : (data?.data?.onHoldTickets?.toString() ?? '0');
        final receivedTickets = isLoading
            ? '0'
            : (data?.data?.resolvedTickets?.toString() ?? '0');
...
// Debug method to check API response (add this temporarily):
  void _debugApiResponse() {
    final apiProvider = Provider.of<TechEngineerApiProvider>(context, listen: false);
