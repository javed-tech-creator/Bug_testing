...
class TechEngineerAssignedTicketListScreen extends StatefulWidget {
  const TechEngineerAssignedTicketListScreen({Key? key}) : super(key: key);

  @override
  State<TechEngineerAssignedTicketListScreen> createState() => _TechEngineerAssignedTicketListScreenState();
}

class _TechEngineerAssignedTicketListScreenState extends State<TechEngineerAssignedTicketListScreen>
  ...
  void _loadDevices() {
    ...
    provider.getAllTechMangerHelpDeskTicketList(context, _currentPage, _limit).then((_) {
      if (provider.getAllTechEngineerTicketListModelResponse?.success == true) {
        setState(() {
          if (_currentPage == 1) {
            _allDevices = provider.getAllTechEngineerTicketListModelResponse!.data?.data ?? [];
          } else {
            _allDevices.addAll(provider.getAllTechEngineerTicketListModelResponse!.data?.data ?? []);
          }
          _applySearch();
        });
  ...
  void _loadMoreDevices() {
    final provider = Provider.of<TechEngineerApiProvider>(context, listen: false);
    final response = provider.getAllTechEngineerTicketListModelResponse;
    ...
    if (_searchQuery.isEmpty) {
      ...
      _filteredDevices = _allDevices.where((ticket) {
        return (ticket.priority?.toLowerCase().contains(_searchQuery) ?? false) ||
            ...
            (ticket.raisedBy?.role?.toLowerCase().contains(_searchQuery) ?? false) ||
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // Content
          Expanded(
            child: Consumer<TechEngineerApiProvider>(
                    ...
                    _buildBottomSheetSection(
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildDetailRow('Role', ticket.raisedBy?.role ?? 'N/A', Icons.work_outline),
                          // _buildDetailRow('Assigned To', ticket.raisedBy?.assignedTo ?? 'Unassigned', Icons.assignment_ind_outlined),
                          // if (ticket.employeeId != null)
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () async {
                            ...
                            try {
                              final provider = Provider.of<TechEngineerApiProvider>(context, listen: false);
                              await provider.updateTechEngineerAssignedTicketStatus(
                                  context, statusBody, ticketId);
                              if (provider.updateTechEngineerAssignStatusModelResponse?.success == true) {
                                _refreshDevices();
                              }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          //     //   ),
          //     //   child: Text(
          //     //     history.updatedBy.toString() ?? 'Unknown Status',
          //     //     style: TextStyle(
          //     //       fontSize: 12,
          //     //       fontWeight: FontWeight.w600,
          //     //       color: Colors.black,
          ...
          //   ],
          // ),
          if (history.updatedBy?.name != null) ...[
            const SizedBox(height: 12),
            Text(
              history.updatedBy?.name.toString() ?? "N/A",
              style: const TextStyle(
                fontSize: 14,
                color: Colors.black87,
                height: 1.4,
