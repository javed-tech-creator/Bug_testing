...
import 'package:dss_crm/tech_module/tech_employee/screen/licence_software/tech_engineer_licence_software__list_card_item_widget.dart';
import 'package:dss_crm/tech_module/tech_employee/screen/licence_software/tech_engineer_licence_software_detail_bottom_sheet_widget.dart';
import 'package:dss_crm/tech_module/tech_employee/screen/my_assets/tech_engineer_asset__list_card_item_widget.dart';
import 'package:dss_crm/tech_module/tech_employee/screen/my_assets/tech_engineer_asset_detail_bottom_sheet_widget.dart'
    show TechEngineerAssetDetailBottomSheet;
...
class TechEngineerLicenceSoftwareManagementScreen extends StatefulWidget {
  const TechEngineerLicenceSoftwareManagementScreen({Key? key})
      : super(key: key);

  @override
  State<TechEngineerLicenceSoftwareManagementScreen> createState() =>
      _TechEngineerLicenceSoftwareManagementScreenState();
}

class _TechEngineerLicenceSoftwareManagementScreenState
    extends State<TechEngineerLicenceSoftwareManagementScreen> {
  ...
  void initState() {
    ...
    // Load assets after build is complete
  ...
  Future<void> loadAssets({bool isInitial = false}) async {
    ...
    try {
      await provider.getAllTechEngineerLicenceSoftwareList(
        context,
        _currentPage,
        _itemsPerPage,
      );

      // The API seems to return asset data, not license data
      // You might need to change this to the correct license API call
      final response = provider.getAllTechEngineerLicenceSoftwareListModelResponse;
        ...
        filteredList = filteredList.where((asset) {
          ...
          return (asset.softwareName?.toLowerCase().contains(searchLower) ?? false) ||
              ...
              (asset.assignedTo?.name?.toLowerCase().contains(searchLower) ??
                  false) ||
              (asset.assignedTo?.department?.toLowerCase().contains(
                searchLower,
              ) ??
                  false) ;
        }).toList();
                ...
                child: Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'Search licenses by software name, ID, version...',
          ...
          Row(
            children: [
              Container(
                ...
                child: Text(
                  'Showing ${_filteredAssets.length} of ${_assets.length} licenses',
                  style: TextStyle(
  ...
  Widget _buildAssetsList() {
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                //   ElevatedButton.icon(
                //     onPressed: () {
                //       // Navigate to the screen to add a new license
                //       Navigator.push(
                //         context,
                //         MaterialPageRoute(builder: (_) => const TechMangerAddLicenseScreen()),
                //       ).then((_) {
                //         // Reload licenses after returning from the screen
                //         loadAssets();
                //       });
                //     },
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        // Then return the license card for all other items
        final asset = _filteredAssets[index];
        return TechEngineerLicenceSoftwareCard(
          license: asset,
  ...
  void _showAssetDetails(Data asset) {
    showModalBottomSheet(
      ...
      builder: (context) => TechEngineerLicenceSoftwareDetailBottomSheet(
        license: asset,
        onLicenseDeleted: () async {
